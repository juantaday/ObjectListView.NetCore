...
namespace BrightIdeasSoftware.Design
{
    ...
    public class ObjectListViewDesigner : ControlDesigner
    {
        ...
        public override DesignerActionListCollection ActionLists {
            get {
                ...
                if (actionLists == null)
                {
                    ...
                    actionLists.Add(new ListViewActionListAdapter(Component));
                }
                else if (actionLists.Count > 0 && !(actionLists[0] is ListViewActionListAdapter)) {
        ...
        {
            private ObjectListView control;
            public ListViewActionListAdapter(IComponent component) : base(component)
            {
                control = component as ObjectListView;
            }
    ...
    /// <summary>
    /// Editor de colección para columnas OLV
    /// </summary>
    public class OLVColumnCollectionEditor : CollectionEditor
    {
        public OLVColumnCollectionEditor(Type type) : base(type)
        {
        }
        ...
        {
            return new Type[] { typeof(OLVColumn) };
        }

        protected override object CreateInstance(Type itemType)
        {
            if (itemType == typeof(OLVColumn))
            {
                var column = new OLVColumn();

                // Obtener un nombre único para la columna
                if (Context?.Instance is ObjectListView olv)
                {
                    int counter = 1;
                    ...
                    //    counter++;
                    //} while (olv.Columns.Any(c => c.Text == uniqueName));

                    //column.Text = uniqueName;
                    column.Width = 100;
                }
                else
                {
        ...
        {
            // Llamar al editor base
            object result = base.EditValue(context, provider, value);

            // Notificar cambios al diseñador
            if (provider?.GetService(typeof(IComponentChangeService)) is IComponentChangeService changeService &&
                context?.Instance is ObjectListView olv)
            {
                changeService.OnComponentChanged(olv,
                    TypeDescriptor.GetProperties(olv)["Columns"], null, olv.Columns);
            }
    ...
    /// <summary>
    /// Editor personalizado para la colección AllColumns
    /// </summary>

    public class CustomColumnCollectionEditor : CollectionEditor
    {
        public CustomColumnCollectionEditor(Type type) : base(type)
        {
        }
            ...
            if (itemType == typeof(OLVColumn))
            {
                ...
                if (context?.Instance is ObjectListView olv)
                {
                    ...
                    do
                    {
                        uniqueName = baseName + counter;
                        counter++;
                    } while (olv.AllColumns.Any(c => c.Name == uniqueName));

                    column.Name = uniqueName;
                    column.Text = "Column " + (counter - 1);
                }
                else
                {
                    column.Name = "olvColumn1";
                    column.Text = "New Column";
                }
        ...
        // CRÍTICO: Este método debe forzar la generación de código
        public override object EditValue(ITypeDescriptorContext context, IServiceProvider provider, object value)
        {
            if (context?.Instance is ObjectListView olv)
            {
                // Guardar estado original
                var originalColumns = new List<OLVColumn>(olv.AllColumns);

                // Marcar como inicializando para evitar actualizaciones prematuras
                olv.BeginInit();

                try
                {
                    // Editar la colección
                    object result = base.EditValue(context, provider, olv.AllColumns);

                    // CRÍTICO: Forzar la generación de código para cada columna
                    if (provider?.GetService(typeof(IDesignerHost)) is IDesignerHost host)
                    {
                        // Crear componentes en el diseñador para cada nueva columna
                        foreach (var column in olv.AllColumns)
                        {
                            if (!string.IsNullOrEmpty(column.Name) && column.Site == null)
                            {
                                // Agregar la columna como componente al contenedor del diseñador
                                host.Container?.Add(column, column.Name);
                            }
                        }
                    }
                    ...
                    if (provider?.GetService(typeof(IComponentChangeService)) is IComponentChangeService changeService)
                    {
                        changeService.OnComponentChanging(olv, TypeDescriptor.GetProperties(olv)["AllColumns"]);
                        changeService.OnComponentChanged(olv, TypeDescriptor.GetProperties(olv)["AllColumns"],
                            originalColumns, olv.AllColumns);
                    }
