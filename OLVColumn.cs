...
namespace BrightIdeasSoftware
{
    ...
    public partial class OLVColumn : ColumnHeader, IComponent
    {
        ...
        public OLVColumn(string text) : this()
        {
            this.Text = text;
        }
        ...
        {
            get => textAlign; // Usando el operador de coalescencia nula
            set
            {
                textAlign = value;
        ...
        private HorizontalAlignment textAlign = HorizontalAlignment.Left; // Valor predeterminado
        /// <summary>
    ...
    /// <summary>
    /// Convertidor de tipos para OLVColumn
    /// </summary>
    public class OLVColumnConverter : TypeConverter
    {
         ...
         object? value, Type destinationType)
        {
            if (value is OLVColumn column)
            {
                ...
                {
                    // Generar código para el constructor apropiado
                    if (!string.IsNullOrEmpty(column.AspectName))
                    {
                        ConstructorInfo? ctor = typeof(OLVColumn).GetConstructor(
                            new Type[] { typeof(string), typeof(string) });
                        if (ctor != null)
                        {
                            return new InstanceDescriptor(ctor,
                                new object[] { column.Text, column.AspectName });
                        }
                    }
                    else if (!string.IsNullOrEmpty(column.Text))
                    {
                        ConstructorInfo? ctor = typeof(OLVColumn).GetConstructor(
                            new Type[] { typeof(string) });
                        if (ctor != null)
                        {
                            return new InstanceDescriptor(ctor, new object[] { column.Text });
                        }
                    }

                    // Constructor sin parámetros como fallback
                    ConstructorInfo? defaultCtor = typeof(OLVColumn).GetConstructor(Type.EmptyTypes);
                    if (defaultCtor != null)
                    {
                        return new InstanceDescriptor(defaultCtor, Array.Empty<object>());
                    }
                }
            }
        ...
        {
            return TypeDescriptor.GetProperties(typeof(OLVColumn), attributes);
        }
